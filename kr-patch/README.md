...
구조 조사 내용(텍스트 위치, 인코딩, 왜 이런 설계가 됐는지)은 [`docs/NOTES.md`](docs/NOTES.md)를
...
- **대사** / **이름·라벨** 두 탭. 검색창으로 원문·수정문 텍스트 검색 가능(오프셋은
  `Ctrl+F`로 페이지 내 찾기 — 페이징 없이 전체 목록이 한 번에 로드됨).
- 각 행의 "수정" 칸에 교정문을 입력하고 **저장** 버튼을 누르면 그 즉시
  `translation.json`에 반영된다(행 단위 즉시 저장 — 브라우저가 죽어도 이미 누른 건 안전).
- 바이트 길이 표시: `대사`는 원문과 정확히 같은 바이트 길이여야 하고(포인터는 없지만
  구조상 다른 데이터가 밀릴 위험이 있어 길이 변경 금지 — `docs/NOTES.md` 참조),
  `이름·라벨`은 짧아진 교정문을 전각공백(`　`)으로 자동 패딩해준다(길어지는 건 여전히
  안 됨 — 슬롯 폭 초과). `폰트` 탭은 다른 개념 — Win32 `LOGFONT`의 `lfFaceName`(고정
  32바이트 NUL 패딩 버퍼, `docs/NOTES.md` 참조)이라 정확히 일치할 필요 없이 **32바이트
  이하면 무엇이든** 들어간다(패딩도 자동으로 NUL로 처리됨).
- 우측 "JP 검색" 패널: 이름·키워드로 일본어 원문을 전후 문맥과 함께 검색해서 대조용으로
  참고(KR·JP는 텍스트 순서가 서로 달라서 자동 대응은 불가능 — 검증된 사실, `docs/NOTES.md`
  참조).
