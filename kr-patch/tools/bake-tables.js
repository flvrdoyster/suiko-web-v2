...
// 이 표시의 쓸모는 두 가지다.
//
// (1) **단위 안 재분배가 가능한 자리**. 대부분의 대사는 relocation이 가리키는 "장면 진입점"
//     에서 `@`를 세며 순차로 읽히므로 한 줄을 늘리면 그 뒤 전부가 밀려 참조가 깨진다(그래서
//     build()가 원본 바이트 길이를 강제한다 — NOTES.md 3.3). 그런데 일부 구간은 **연속된
//     4바이트 포인터 슬롯 테이블**이 각 단위의 시작을 개별적으로 가리킨다. 그런 구간은 한
//     단위 안에서(시작점에서 순차로 읽히므로) 포인터를 건드리지 않고도 줄 사이에 바이트를
//     재분배할 수 있다 — 줄바꿈 위치를 다시 잡는 용도.
//     ⚠ **총량은 늘릴 수 없다.** 단위 합계는 원본과 같아야 하고, 블록 뒤에 빈 공간도 없다
//     (실측: 45개 연속 구간의 span 합 == 사용량 합, 여유 0). 순증가는 PE 재작성이 필요하며
//     진행하지 않기로 한 사안이다 — NOTES.md 3.3의 "순증가 불가" 참고.
//
// (2) **발화 대사가 아닌 텍스트의 식별자**. 실측상 이 구간은 「로 시작하는 줄이 0.6%
//     (8/1,434)뿐인 반면 나머지 대사 구간은 27.0%(3,617/13,378)다 — 대사는 씬 스크립트가
//     순차로 훑는 반면, 독백·설명류는 임의 접근이 필요해 포인터를 쓰기 때문. 내용은
//     설명·시스템 708줄(기술/아이템 설명·인명·지명·메뉴·장 제목 등) + 독백·나레이션 726줄.
//     에디터의 `독백·설명` 탭이 이 필드로 걸러낸다(말투 규칙이 다른 묶음 — GUIDE 2-4).
//
