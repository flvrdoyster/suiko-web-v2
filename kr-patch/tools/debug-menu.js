#!/usr/bin/env node
// debug-menu.js — add a third item to HWANSE.EXE's title menu, wired to the leftover debug
// scenario-warp menu that ships (unreachable) in the retail KR build.
//
// Usage:
//   node kr-patch/tools/debug-menu.js [--exe path] [--out path] [--target select|scen9]
...
function fail(msg) {
  console.error('debug-menu.js: ' + msg);
  process.exit(1);
}
...
for (let o = data.rawOff; o < DATA_END - 4; o += 4) {
  ...
  else console.warn(`debug-menu.js: 경고 — file ${o}의 0x4A3440 참조가 GOTO/CALL이 아님, 건너뜀`);
}
...
// --- Optional: emit the patch as data, for docs/suiko-debug.js to apply in the browser ----
...
if (emitPatch) {
  ...
  const patch = {
    note:
      `Generated by kr-patch/tools/debug-menu.js --mode ${mode} --emit-patch. ` +
      (mode === 'direct'
      ...
      ' Applied to the in-memory disk image before boot by docs/suiko-debug.js;' +
      ' see NOTES.md 3.4.',
    mode,
    target: 'HWANSE.EXE',
    dir: 'GENSE',
...
console.log(`debug-menu.js: ${path.relative(ROOT, outPath)}  [--mode ${mode}]`);
