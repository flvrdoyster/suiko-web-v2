...
function extractDirFiles(bytes, dirPath) {
  ...
  for (const e of listDir(ctx, cluster)) {
    ...
    files.push({ name, data: readFileEntry(ctx, e) });
  }
  return files;
}
...
// Updates the directory entry's size field. Mutates ctx.img.
function writeFileInPlace(ctx, entry, data) {
  const chain = clusterChain(ctx, entry.firstCluster);
...
function injectDirFiles(bytes, dirPath, files) {
  ...
  for (const f of files) {
    ...
    writeFileInPlace(ctx, target, f.data instanceof Uint8Array ? f.data : new Uint8Array(f.data));
  }
