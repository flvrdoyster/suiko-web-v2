...
- **번역 전량 재검토**: 미수정·미확정 대사를 원문(JP)과 1:1 대조해 결함 후보를 추리는 전수 검토.
  ...
  - **기계 감지 필터**: 에디터 필터 드롭다운이 상시 후보를 추린다 — 문장부호 계열(부호
    추가+반복 / 반복 증폭 / 끝 어조 불일치 / 더듬음 공백)과 오역 계열(숫자 불일치 / 대립쌍
    반전 / 이름 누락 / 분량 축소). 판정 로직은 `editor.html`의 `AUTO_FILTERS`가 정본이고,
    수정하거나 확정한 줄은 자동 제외돼 잔여 건수가 줄어든다. 건수가 0이 된 옵션은 드롭다운에서
    자동으로 사라진다(예전엔 손으로 지웠음).
